    ...
    public List<Mat> calculateExtrinsics(Mat opticalFlow) {
        ...
        for (int y = 0; y < height; y += subsamplingRatio) {
            ...
            for (int x = 0; x < width; x += subsamplingRatio) {
                ...
                Log.d(TAG, "(" + x + ", " + y + ") -> (" + (x + row_dx[x]) + ", " + (y + row_dy[x]) + ")");
            }
        }
        ...
        Calib3d.decomposeEssentialMat(essentialMatrix, R1, R2, t_d);
        //Mat essentialMatrix = Calib3d.findEssentialMat(cameraAPoints, cameraBPoints);
        //Calib3d.recoverPose(essentialMatrix, cameraAPoints, cameraBPoints, R, t);

        Log.d(TAG, essentialMatrix.dump());
        Log.d(TAG, R.dump());
        Log.d(TAG, R1.dump());
        Log.d(TAG, R2.dump());
        Log.d(TAG, t.dump());
